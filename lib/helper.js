...
var validateType = function(msg, callback) {
   ...
   for ( var i in type['@context']) {
      ...
      if(type['@context'].hasOwnProperty(i)) {
         ...
         if (-1 === allowedTypes.indexOf(entry['@openi_type'].toLowerCase())) {
   ...
   var request = https.get(type, function(res) {
      ...
      res.on('end', function () {
         ...
         try{
            ...
            return callback( {valid:false, message: type + ' cannot be found.'});
         }
         ...
         if (type_json["@id"] == type_id ){
            ...
            return callback( {valid:false, message: type + ' is not a valid type.'});
         }
      });
         ...
         return callback( {valid:false, message: type + ' could not be found.'});
      });
}
...
var genPostMessage = function(msg, senderToDao, senderToClient, terminal_handler) {
   ...
   validateType(msg, function(validCheck){
      ...
      if (!validCheck.valid) {
         ...
         senderToClient.send(msg.uuid, msg.connId, zmq.status.BAD_REQUEST_400, zmq.standard_headers.json, {'error' : 'Type object isn\'t properly structured : ' + validCheck.errors.join(" ")});
