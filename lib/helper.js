...
var allowedTypes = ['boolean', 'int', 'float', 'string', 'date', 'timestamp', 'hexadecimal', 'base64'];
...
var validateType = function(msg, callback) {
   ...
   if ( !('@reference' in  type) || '' === type['@reference']) {
      callback( {valid:false, message:'There is no _reference property'});
   }
   if ( !('@context' in  type) || !Array.isArray(type['@context']) || 0 === type['@context'].length ){
      callback( {valid:false, message:'There is no _context property'});
   }
   ...
   for ( var i in type['@context']) {
      ...
      if(type['@context'].hasOwnProperty(i)) {
         ...
         if ( !('@property_name' in  entry) || '' === entry['@property_name']) {
            callback( {valid:false, message:'There is no _property_name parameter in entry ' + i});
         }

         if ( !('@property_context' in  entry) ) {
            callback( {valid:false, message:'There is no _property_context parameter in entry ' + i});
         }

         if ( !( '@id' in entry['@property_context']) || '' === entry['@property_context']['@id']) {
            callback( {valid:false, message:'There is no _id parameter in context of the entry ' + i});
         }

         if ( !( '@openi_type' in entry['@property_context']) ) {
            callback( {valid:false, message:'There is no _openi_type parameter in context of the entry ' + i});
         }

         if (-1 === allowedTypes.indexOf(entry['@property_context']['@openi_type'].toLowerCase())) {

            var type_val = entry['@property_context']['@openi_type']

            if ( 0 === type_val.indexOf('t_') ){
               subTypes.push(type_val)
            }
            else{
               callback( {valid:false, message: entry['@property_context']['@openi_type'].toLowerCase() + ' is not a valid type.'});
            }
         }
      }
   }

   if (0 === subTypes.length){
      callback( {valid:true})
   }
   else{
   ...
   var request = https.get(type, function(res) {
      ...
      res.on('end', function () {
         ...
         try{
            ...
            callback( {valid:false, message: type + ' cannot be found.'});
         }
         ...
         if (type_json["@id"] == type_id ){
            ...
            callback( {valid:false, message: type + ' is not a valid type.'});
         }
      });
         ...
         callback( {valid:false, message: type + ' could not be found.'});
      });
}
...
var genPostMessage = function(msg, senderToDao, senderToClient, terminal_handler) {
   ...
   validateType(msg, function(validCheck){
      ...
      if (!validCheck.valid) {
         ...
         senderToClient.send(msg.uuid, msg.connId, zmq.status.BAD_REQUEST_400, zmq.standard_headers.json, {'error' : 'Type object isn\'t properly structured : ' + validCheck.message});
